...
final class Cbor_Encoder
{
    ...
    private function encode_value(mixed $value): string
    {
        switch (gettype($value)) {
            case 'string':
                ...
                return $this->encode_text_string($value);
            case 'array':
            ...
            case 'object':
                ...
                if ($value instanceof DateTimeInterface) {
                    ...
                    if ($micro === 0) {
                        ...
                        return "\xc1" . $this->encode_integer($timestamp);
                    }
                    return "\xc1\xfb" . pack('E', $timestamp + $micro / 1000000.0);
                }
                throw new Cbor_Exception('Cannot encode object of type: ' . $value::class);
            default:
                throw new Cbor_Exception('Cannot encode value of type: ' . gettype($value));
        }
    }
    /**
     * Encode an integer (major type 0 or 1)
     */
    private function encode_integer(int $value): string
    {
        if (isset(self::INT_CACHE[$value])) {
            return self::INT_CACHE[$value];
        }
     ...
     */
    private function encode_text_string(string $value): string
    {
        $len = strlen($value);
        if ($len < 24) {
            return chr(0x60 | $len) . $value;
     ...
     */
    private function encode_array(array $value): string
    {
        $count = count($value);
        if ($count < 24) {
     ...
     */
    private function encode_map(array $value): string
    {
        $count = count($value);
        if ($count < 24) {
     ...
     */
    public function encode_empty_map(): string
    {
        return "\xa0";
    }
    /**
     ...
     */
    public function encode_empty_indefinite_map(): string
    {
        return "\xbf\xff";
    }
}